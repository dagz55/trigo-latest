...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        -- Create the policy
        EXECUTE 'CREATE POLICY "Triders can view bookings assigned to them" ON public.bookings FOR SELECT USING (auth.uid() IN (SELECT user_id FROM public.triders WHERE id = trider_id))';
    END IF;
